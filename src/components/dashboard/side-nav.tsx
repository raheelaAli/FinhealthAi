  ...
  return (
    <aside className="w-60 bg-white border-r border-gray-200 flex flex-col h-full shrink-0">
      <div className="px-5 py-4 border-b border-gray-200">
        <Logo size="md" />
        <p className="text-xs text-emerald-600 capitalize mt-1 pl-0.5">{user.role.toLowerCase()} plan</p>
      </div>
      <nav className="flex-1 px-3 py-4 space-y-0.5">
        ...
        {user.role === "ADMIN" && (
          <div className="pt-2 mt-2 border-t border-gray-200">
            <Link href="/admin"
              className="flex items-center gap-3 px-3 py-2 rounded-lg text-sm text-red-600 hover:bg-red-50 transition-colors font-medium">
      ...
      <div className="px-3 py-4 border-t border-gray-200">
        <div className="flex items-center gap-3 px-3 py-2 mb-1">
          <div className="w-7 h-7 bg-emerald-100 rounded-full flex items-center justify-center shrink-0">
            <span className="text-emerald-700 text-xs font-semibold">{user.name?.[0]?.toUpperCase() ?? "U"}</span>
          </div>
